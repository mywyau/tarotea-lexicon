...
SYSTEM_PROMPT = (
    ...
    "Your task is to judge whether each Chinese example sentence sounds natural "
    "and appropriate for Hong Kong Cantonese usage.\n\n"

    "Review each example carefully and compare all three fields:\n"
    "- Chinese sentence\n"
    "- Jyutping\n"
    "- English meaning\n\n"
    ...
    "Rules:\n"
    "- Do not flag a sentence only because a more colloquial or polished alternative exists.\n"
    "- Formal, slightly compressed, or textbook-style wording can still be approved if it is natural enough and not misleading.\n"
    "- Distinguish between 'not the best sentence' and 'actually unnatural Cantonese'.\n"
    "- Only return likely_unnatural when a native Cantonese speaker would find the sentence noticeably awkward, unnatural, misleading, or suspicious.\n"
    ...
    "- Write all reasons and suggestions in English only.\n"
    "- Return strict JSON only.\n\n"

    "Before deciding, check whether the sentence could plausibly be said by a native Hong Kong Cantonese speaker in real life. "
    "Then make the most accurate conservative judgment."
)
...
def extract_output_text_from_response_body(body: dict) -> str:
    """
    Batch output is raw JSON, not the SDK response object.
    Try the top-level output_text first, then fall back to walking output items.
    """
    ...
    for ex_review in review.get("examples", []):
        if ex_review.get("decision") != "approved":
            ...
            issues.append({
                ...
                "suggestedSentence": ex_review.get("suggestedSentence") or "",
                "suggestedJyutping": ex_review.get("suggestedJyutping") or "",
                "suggestedMeaning": ex_review.get("suggestedMeaning") or "",
...
def submit_batch(batch_input_path: Path) -> str:
    ...
    batch = client.batches.create(
        ...
        metadata={
            "job": "example-naturalness-review",
            "model": OPENAI_MODEL,
        },
    )
...
def collect_results(batch_id: str) -> None:
    ...
    # Start with local parse errors from the prepare phase.
    ...
    # Load successful/failed request results from batch output.
    ...
    # Also ingest explicit batch error file rows if present and not already covered.
    ...
    # Any queued item not seen in outputs/errors becomes a safety fallback.
    for custom_id, item in manifest.items():
